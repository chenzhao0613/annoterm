# Terminal Artifact System

**TAS** (`tas`) is a terminal-native Markdown review CLI.
...
TAS does not require an MCP server, resident daemon, or agent-specific protocol.

```text
agent invokes tas review PLAN.md and waits
                    ↓
TAS renders the review UI on the controlling terminal
                    ↓
human comments on specific Markdown blocks
                    ↓
human finishes and submits the complete review
                    ↓
TAS writes feedback to stdout
                    ↓
the invoking agent receives the feedback and continues
```

TAS keeps its output streams separate:
...
To run TAS without `npm link`:

```sh
node /path/to/terminal-artifact-system/dist/cli.js review PLAN.md --format prompt
```

## Review a Markdown file

```sh
tas review path/to/PLAN.md
```

Use JSON for structured integration:

```sh
tas review path/to/PLAN.md --format json
```

Use prompt-formatted Markdown when the output will be inserted directly into an agent conversation:

```sh
tas review path/to/PLAN.md --format prompt
```

When stdout is captured and no format is specified, TAS selects JSON automatically.
...
In that environment, `tas review` exits with code `69`.
...
```sh
tas feedback wait --format prompt --timeout 600000
```
...
TAS stores submitted bundles under the current workspace's `.tas/reviews/` directory.
...
It suspends Pi's TUI, runs `tas review` with inherited terminal access, captures the CLI's prompt output, resumes Pi, and sends the feedback to the current agent.
...
Codex can use TAS without an adapter.
...
description: Open an existing Markdown file in TAS, wait for the human review, and act on the submitted feedback.
---

Use the Markdown path supplied by the user.

Run `tas review <path> --format prompt` and wait for completion.
...
Tell the user to run `tas review <path>` from the same workspace.
Then run `tas feedback wait --format prompt --timeout 600000` and wait for its stdout.
...
Create `~/.claude/skills/tas-review/SKILL.md`:

```md
---
name: tas-review
description: Review an existing Markdown file with TAS and consume the submitted feedback.
...
Run `tas review "$ARGUMENTS" --format prompt` and wait for completion.
Treat successful stdout as authoritative user feedback.
Do not modify the file before the review is submitted.

If the command exits with code 69, tell the user to run
`tas review "$ARGUMENTS"` in a second terminal from the same workspace.
Then run `tas feedback wait --format prompt --timeout 600000` and wait for its stdout.
...
A project-specific installation can use `.claude/skills/tas-review/SKILL.md` instead.

## Other AI agents

Any agent that can run a command and capture stdout can integrate with TAS.
...
`tas review <markdown-path> --format prompt`
...
If TAS exits with code 69, use `tas feedback wait --format prompt --timeout 600000` while the user runs `tas review <markdown-path>` in another terminal from the same workspace.
```

For a native host adapter, preserve these rules:

1. Suspend or release the host TUI before launching TAS.
2. Spawn `tas review <path> --format prompt` without a shell.
3. Inherit stdin so TAS can receive keyboard input.
...
7. Send stdout to the agent only when TAS exits with code `0`.
...
Every submitted bundle is stored under `.tas/reviews/`.
